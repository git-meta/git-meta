...
#
# Commit messages use the legacy `git-meta: serialize` prefix on purpose to
# exercise the parser's backward-compatibility path:
#   Commit 1: "git-meta: serialize (1 changes)\n\nA\tproject\told_key"
#   Commit 2: "git-meta: serialize (1 changes)\n\nA\tproject\ttesting"
set -eu -o pipefail
...
COMMIT1=$(printf 'git-meta: serialize (1 changes)\n\nA\tproject\told_key' \
...
COMMIT2=$(printf 'git-meta: serialize (1 changes)\n\nA\tproject\ttesting' \
